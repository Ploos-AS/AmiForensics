...
OUT_DIR="${1:-build/fs-uae/native-portview}"
mkdir -p "$OUT_DIR"
...
cp PortView "$OUT_DIR/PortView"
file "$OUT_DIR/PortView" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/PortView" | tee "$OUT_DIR/PortView.sha256"
...
printf 'STATUS=PASS\nGATE=M2_5B_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/PortView" | tee "$OUT_DIR/result.txt"
