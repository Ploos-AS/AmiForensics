...
OUT_DIR="${1:-build/fs-uae/native-memscan}"
mkdir -p "$OUT_DIR"
...
cp MemScan "$OUT_DIR/MemScan"
file "$OUT_DIR/MemScan" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/MemScan" | tee "$OUT_DIR/MemScan.sha256"
...
printf 'STATUS=PASS\nGATE=M3_1B_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/MemScan" | tee "$OUT_DIR/result.txt"
