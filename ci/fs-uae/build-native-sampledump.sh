...
OUT_DIR="${1:-build/fs-uae/native-sampledump}"
mkdir -p "$OUT_DIR"
...
cp SampleDump "$OUT_DIR/SampleDump"
file "$OUT_DIR/SampleDump" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/SampleDump" | tee "$OUT_DIR/SampleDump.sha256"
...
printf 'STATUS=PASS\nGATE=M3_4_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/SampleDump" | tee "$OUT_DIR/result.txt"
