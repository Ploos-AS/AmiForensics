...
OUT_DIR="${1:-build/fs-uae/native-processview}"
mkdir -p "$OUT_DIR"
...
cp ProcessView "$OUT_DIR/ProcessView"
file "$OUT_DIR/ProcessView" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/ProcessView" | tee "$OUT_DIR/ProcessView.sha256"
...
printf 'STATUS=PASS\nGATE=M2_4B_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/ProcessView" | tee "$OUT_DIR/result.txt"
