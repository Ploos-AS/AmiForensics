...
OUT_DIR="${1:-build/fs-uae/native-diskwatch}"
mkdir -p "$OUT_DIR"
...
cp DiskWatch "$OUT_DIR/DiskWatch"
file "$OUT_DIR/DiskWatch" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/DiskWatch" | tee "$OUT_DIR/DiskWatch.sha256"
...
printf 'STATUS=PASS\nGATE=M4_4B_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/DiskWatch" | tee "$OUT_DIR/result.txt"
