...
OUT_DIR="${1:-build/fs-uae/native-patchview}"
mkdir -p "$OUT_DIR"
...
cp PatchView "$OUT_DIR/PatchView"
file "$OUT_DIR/PatchView" | tee "$OUT_DIR/file.txt"
sha256sum "$OUT_DIR/PatchView" | tee "$OUT_DIR/PatchView.sha256"
...
printf 'STATUS=PASS\nGATE=M2_2_NATIVE_BEBBO_BUILD\nIMAGE=%s\nBINARY=%s\n' \
  "$IMAGE" "$OUT_DIR/PatchView" | tee "$OUT_DIR/result.txt"
